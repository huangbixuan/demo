...
public class AppStoreProductServiceImpl extends ServiceImpl<StoreProductMapper,StoreProductDO> implements AppStoreProductService {
    ...
    public List<AppCategoryRespVO> getGoodsList(AppStoreProductQueryParam productQueryParam) {
        String StrShopId = productQueryParam.getShopId().toString();
        //redis统计今日访客
        if (RedisUtil.hasKey(DAY_COUNT_KEY)) {
            if (RedisUtil.hHasKey(DAY_COUNT_KEY, StrShopId)) {
                RedisUtil.hincr(DAY_COUNT_KEY, StrShopId, 1);
            } else {
                RedisUtil.hset(DAY_COUNT_KEY, StrShopId, 1);
            }
        } else {
            RedisUtil.hset(DAY_COUNT_KEY, StrShopId, 1, DateUtils.getRemainSecondsOneDay1(new Date()));
        }
