...
public class MpUserController {
    ...
    @GetMapping("/addAdmin")
    @Operation(summary = "添加通知管理员")
    @Parameter(name = "ids", description = "编号列表", required = true, example = "1024,2048")
    public CommonResult<Boolean> addAdmin(@RequestParam("ids") Collection<Integer> ids) {
        mpUserService.addAdminNotice(ids);
        return success(true);
    }

    @DeleteMapping("/delAdmin")
    @Operation(summary = "移除通知管理员")
    @Parameter(name = "id", description = "编号", required = true)
    public CommonResult<Boolean> deleteAdmin(@RequestParam("id") Integer id) {
        mpUserService.deleteAdminNotice(id);
        return success(true);
    }
