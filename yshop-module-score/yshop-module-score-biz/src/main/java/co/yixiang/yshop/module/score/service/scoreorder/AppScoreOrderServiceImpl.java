...
public class AppScoreOrderServiceImpl extends ServiceImpl<ScoreOrderMapper, ScoreOrderDO> implements AppScoreOrderService {
    ...
    public String submit(Long uid,AppScoreOrderParam appScoreOrderParam) {
       ...
       UserAddressDO userAddressDO =  appUserAddressService.getById(appScoreOrderParam.getAddressId());
        ...
        //判断商品是否需要价格
        Integer havaPaid = ShopCommonEnum.IS_STATUS_0.getValue();
        BigDecimal price = BigDecimal.ZERO;
        //是否是积分兑换
        if(appScoreOrderParam.getIsOrigin().equals(ShopCommonEnum.DEFAULT_1.getValue()) ){
            if(scoreProductDO.getPrice().compareTo(BigDecimal.ZERO) == 0){
                havaPaid = ShopCommonEnum.IS_STATUS_1.getValue();
            }else{
                price = scoreProductDO.getPrice();
            }
        }else{
            if(scoreProductDO.getOtPrice().compareTo(BigDecimal.ZERO) == 0){
                havaPaid = ShopCommonEnum.IS_STATUS_1.getValue();
            }else{
                price = scoreProductDO.getOtPrice();
            }
        }
        ...
        ScoreOrderDO scoreOrderDO = ScoreOrderDO.builder()
                ...
                .price(price)
                .totalPrice(price)
                .payType(appScoreOrderParam.getPayType())
                .isOrigin(appScoreOrderParam.getIsOrigin())
                .uid(uid)
                .build();
        //保存
        this.save(scoreOrderDO);
        this.deStockIncSale(scoreProductDO.getId(),Integer.valueOf(appScoreOrderParam.getNum()));
        if(OrderInfoEnum.PAY_STATUS_1.getValue().equals(havaPaid)){
            //减去积分
            ...
            billService.expend(uid, "积分兑换",
                    ...
                    scoreProductDO.getScore() + "积分兑换商品",null);
            return "0";
        }
    ...
    public List<AppScoreOrderVO> orderList(Long uid, int type, int page, int limit) {
        ...
        switch (OrderStatusEnum.toType(type)) {
            ...
            //待支付
            case STATUS_3:
                wrapper.eq(ScoreOrderDO::getHavePaid, OrderInfoEnum.PAY_STATUS_0.getValue());
                break;
            //待发货
            case STATUS_0:
    ...
    private AppScoreOrderVO handleOrder(AppScoreOrderVO order) {
        ...
        } else if (OrderInfoEnum.PAY_STATUS_0.getValue().equals(order.getHavePaid())) {
